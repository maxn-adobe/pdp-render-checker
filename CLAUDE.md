...
- **Concurrency is capped ~8** (`config.perf.maxConcurrency`) — higher starves the renderer → *transient false failures*; a **serial retry pass** recovers them. Re-validate with a back-to-back parity run (require `regressions=0`) before raising.
